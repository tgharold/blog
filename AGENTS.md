...
## Commit Guidelines
Keep commit titles to 50 characters or fewer. Allow up to 75 characters per line in the commit body. When creating git commits, include a co-authored-by line that reflects the model and tool currently in use (update dynamically rather than using a static value). The co-authored-by line should always be on a separate line by itself in git commits, never appended to the commit title.
